/*
 *  19T1698W_kadai_4_1.c
 *  必須課題 4-1
 *  3x3のメディアンフィルタを行うプログラム
 */
...
int main(int argc, char *argv[]){
    ...
    if(argc != 2){
        ...
        printf("Usage : ./kadai_4_1 [input filename]\n");
        return 1;
    }

    char *In_filename = argv[1];            //読み込む画像のファイル名
    char *Out_filename = "result-4-1.jpg";  //出力する画像のファイル名
    ...
    int *neihgborhood[3] = {NULL};          //3x3の画素を保存する配列
    ...
    for(c=0;c<3;c++){
        neihgborhood[c] = (int *)malloc(sizeof(int)*9);
                ...
                for(i=-1;i<2;i++){
                    for(j=-1;j<2;j++){
                        if(x+i < 0 || x+i >= imageWidth || y+j < 0 || y+j >= imageHeight){
                            neihgborhood[c][(i+1)+(j+1)*3] = 0;
                        }else{
                            neihgborhood[c][(i+1)+(j+1)*3] = input_data[c][x+i][y+j];
                        }
                    }
                }
                dataOut[(x+y*imageWidth)*3+c] = median(neihgborhood[c],3);
            }
        }
    }
