...
int laplacian_4(int *neighborhood,int filter_size);
int laplacian_8(int *neighborhood,int filter_size);

int main(int argc, char *argv[]){

    if(argc != 2){
        printf("command error \n");
        printf("Usage : ./kadai_4_3 [input filename]\n");
        return 1;
    }

    char *In_filename = argv[1];    //読み込む画像のファイル名
    char *Out_filename[2] = {
        "result-4-3-4.jpg","result-4-3-8.jpg"
    };                              //出力する画像のファイル名
    ...
    for(x=0;x<imageWidth;x++){
        for(y=0;y<imageHeight;y++){
            for(c=0;c<3;c++){
                ...
                dataOut[0][(x+y*imageWidth)*3+c] = laplacian_4(neihgborhood[c],FILTER_SIZE);
                dataOut[1][(x+y*imageWidth)*3+c] = laplacian_8(neihgborhood[c],FILTER_SIZE);
            }
        }
    }
