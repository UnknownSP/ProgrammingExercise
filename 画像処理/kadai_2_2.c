...
int main(void){
    ...
    /*クロマキー画像データの読み込みを行い、人物画像とサイズが違った場合終了*/
    ...
    /*人物画像のRGB値がクロマキー画像のRGBの平均値±Dであった場合出力画像データをRGB各0にする*/
    for(i=0;i<imageSize;i+=3){
        if( dataIn_person[i] <= R_ave+D && dataIn_person[i] >= R_ave-D
            && dataIn_person[i+1] <= G_ave+D && dataIn_person[i+1] >= G_ave-D
            && dataIn_person[i+2] <= B_ave+D && dataIn_person[i+2] >= B_ave-D ){
            dataOut[i] = 0;
            dataOut[i+1] = 0;
            dataOut[i+2] = 0;
        }
