/*
 *  19T1698W_jiyukadai_4_1.c
 *  自由課題 4-1
 *  nxnのメディアンフィルタを行うプログラム
 */
...
int main(int argc, char *argv[]){
    ...
    if(argc != 2){
        ...
        printf("Usage : ./jiyukadai_4_1 [input filename]\n");
        return 1;
    }

    char *In_filename = argv[1];            //読み込む画像のファイル名
    char *Out_filename = "result-j-4-1.jpg";//出力する画像のファイル名
    ...
    /*各画素、各色ごとにFILTER_SIZExFILTER_SIZEのメディアンフィルタを行う*/
    for(x=0;x<imageWidth;x++){
        for(y=0;y<imageHeight;y++){
            for(c=0;c<3;c++){
                ...
                dataOut[(x+y*imageWidth)*3+c] = median(neihgborhood[c],FILTER_SIZE);
            }
        }
    }
